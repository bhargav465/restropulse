...
interface InputsProps {
    ...
    onUpdate: (type: 'OFFER' | 'SPECIAL' | 'MENU', action: 'ADD' | 'DELETE' | 'UPDATE', payload?: any) => void;
}
...
const Inputs: React.FC<InputsProps> = ({ restaurantData, onUpdate }) => {
  ...
  const handleAdd = (type: 'OFFER' | 'SPECIAL' | 'MENU', value?: string) => {
      if (type === 'OFFER' && value) {
          onUpdate('OFFER', 'ADD', value);
      }
      if (type === 'SPECIAL' && value) {
          onUpdate('SPECIAL', 'ADD', value);
      }
      if (type === 'MENU') {
          onUpdate('MENU', 'UPDATE');
      }
      
      // Close via history back
      window.history.back();
      setOfferInput("");
      setSpecialInput("");
  };

  const handleDelete = (type: 'OFFER' | 'SPECIAL', index: number) => {
      onUpdate(type, 'DELETE', index);
  };
