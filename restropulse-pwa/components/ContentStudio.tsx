...
const PostCard: React.FC<PostCardProps> = ({ post, tab, onApprove, onFeedback }) => {
            ...
            <div className={`relative w-full ${isVertical ? 'aspect-[4/5]' : 'h-64'} bg-black group-hover:bg-slate-900 transition-colors`}>
                {!isPlaying && (
                    <img 
                        src={post.thumbnail} 
                        alt="Video Thumbnail" 
                        className={`w-full h-full object-cover ${post.status === 'MISSED_DEADLINE' ? 'grayscale' : ''}`} 
                    />
                )}
                {/* Video Element */}
                <video 
          ...
          return (
              ...
              >
                  <img 
                      ...
                      className={`w-full h-full object-cover transition-opacity duration-300 ${post.status === 'MISSED_DEADLINE' ? 'grayscale' : ''}`}
                  />
  ...
  return (
    <div className={`bg-white rounded-3xl overflow-hidden shadow-sm border mb-6 group transition-all hover:shadow-md ${post.status === 'MISSED_DEADLINE' ? 'border-red-100 opacity-90' : 'border-slate-100'}`}>
...
const ContentStudio: React.FC = () => {
  ...
  const reviewPosts = posts.filter(p => p.status === 'PENDING_APPROVAL' || p.status === 'CHANGES_REQUESTED');
